...
def nw_basic(seq_a, seq_b, gop=-2.0, gep=-2.0, scores={}):
    """
    Align two sequences using a flavour of the Needleman-Wunsch algorithm with
    fixed gap opening and gap extension penalties, attributed to Gotoh (1994).
...
#            x, y = x-1, y-1
    ...
    return matrix[(len(seq_a), len(seq_b))][0]
...
def readBible(fname):
    ...
    for line in f:
        ...
        if line.startswith("#"):
            ...
            for w in arr[1].split(" "):
                w = w.lower()
                non_alpha_word = False
                for ch in w:
                    if ch.isdigit(): non_alpha_word = True
                if len(w) > 1 and not non_alpha_word:
                    d[w] += 1
